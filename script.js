...
// Main application logic
    ...
    document.getElementById('bodyWrapper').classList.toggle('layers-open', !layersSidebar.classList.contains('collapsed'));
    ...
    function highlightLayer(layerId) {
      const g = document.querySelector(`#scalableContent > g[data-layer-id="${layerId}"]`);
    ...
    function addZoneToTable(zoneName, zoneId) {
      ...
      const spotIds = Array.from(allZoneSpots).map(s => s.getAttribute("data-spot-id")).join(", ");
  ...
  function rebuildLayersList() {
    ...
    groups.forEach((g, idx) => {
      ...
      li.draggable = true;
      li.innerHTML =
        `<span class="drag-handle">☰</span><span class="layer-icon">${getLayerIcon(g)}</span><span class="layer-name">${getLayerName(g)}</span>`;
      li.addEventListener("mouseover", () => highlightLayer(g.getAttribute("data-layer-id")));
      li.addEventListener("mouseout", () => removeHighlightLayer(g.getAttribute("data-layer-id")));
  ...
  layersList.addEventListener('contextmenu', e => {
    ...
    if (!li) return;
    e.preventDefault();
    contextTarget = document.querySelector(`#scalableContent > g[data-layer-id="${li.dataset.targetId}"]`);
